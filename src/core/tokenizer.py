...
num=r'(?:(?<!\d)[+-])?\d+(?:[.,:/]\d+)*(?:[.](?!\.)|-[^\W\d_]+)?'
emoticon=r'[=:;8][\'-]*(?:\s?\)+|\s?\(+|\s?\]+|\s?\[+|\sd\b|\sp\b|d+\b|p+\b|s+\b|o+\b|/|\\|\$|\*+)|-\.-|\^_\^|\([\W]+\)|<3|</3|<\\3|\\o/'
word=ur'(?:[*]{2,})?\w+(?:[@­\'-]\w+|[*]+\w+)*(?:[*]{2,})?'
  ...
  for sent in input:
    ...
    for token,start,end in sent:
      if not token[0].isspace():
        token_id+=1
        output+=str(par_id)+'.'+str(sent_id)+'.'+str(token_id)+'\t'+token+'\t'+str(start)+'\t'+str(end)+'\n'
    output+='\n'
  return output

def usage():
  sys.stderr.write('Usage: ./tokenizer.py lang mode\nlang={hr,sr,sl}\nmode={standard,nonstandard}\n')
  sys.exit(1)

if __name__=='__main__':
  if len(sys.argv)!=3:
    usage()
  lang,mode=sys.argv[1:3]
  if lang not in ('hr','sr','sl') or mode not in ('standard','nonstandard'):
    usage()
